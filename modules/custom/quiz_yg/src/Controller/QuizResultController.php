...
class QuizResultController extends ControllerBase {
  ...
  public function getUserQuizResult($quiz_id,$attempt_id) {
    ...
    $account = \Drupal\user\Entity\User::load($value->user_id); 
    $name = $account->getUsername();
    $element['name']=[
      '#markup' => "<p>".$this->t("Name : @name",[
        "@name"=>$name,
      ])."</p>",
    ];
    $element['scores']=[
      '#markup' => "<p>".$this->t("Score : @point out of @out",[
        "@point"=>$tot,
        "@out" => $out
      ])."</p>",
    ];
    $element['attempted']=[
      '#markup' => "<p>".$this->t("Attempt Time : @time",["@time"=>format_date($created)])."</p>",
    ];
    foreach ($result as $key => $value) {
      foreach ($value as $key1 => $value1) {
        $question=Quiz::load($value1['question_id']);
  ...
  private function getMultichoiceResult(array $result){
    global $base_url;
    $quiz=Quiz::load($result['question_id']);
  ...
  private function getMatchingResult(array $matches){
    ...
    foreach ($matches as $key => $value) {
      $quiz=Quiz::load($value['question_id']);
      $yanswer=$value['answer'];
      $manswer=$quiz->field_answer->value;
      $choice=$quiz->field_question->value;
      if($manswer==$yanswer){
        $result['correct']=1;
