...
namespace Drupal\quiz_yg\Plugin\EntityReferenceSelection;
 ...
 *   group = "default",
 *   entity_types = {"quiz_yg"},
 *   weight = 0
 * )
 */
class QuizSelection extends DefaultSelection {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['negate'] = [
      ...
      '#title' => $this->t('Which Quiz types should be allowed?'),
    ...
    if ($bundle_options) {
      $form['target_bundles_drag_drop'] = [
        ...
        '#prefix' => '<h5>' . $this->t('Quiz types') . '</h5>',
        '#suffix' => '<div class="description">' . $this->t('Selection of Quiz types for this field. Select none to allow all Quiz types.') . '</div>',
      ];
    ...
    if (!count($bundle_options)) {
      $form['allowed_bundles_explain'] = [
        '#type' => 'markup',
        '#markup' => $this->t('You did not add any Quiz types yet, click <a href=":here">here</a> to add one.', [':here' => Url::fromRoute('quiz_yg.type_add')->toString()]),
      ];
    }
   ...
   *
   * @return array
   *   A list of arrays keyed by the quiz_yg type machine name
  ...
  public function getSortedAllowedTypes() {
    ...
    $bundles = $this->entityTypeBundleInfo->getBundleInfo('quiz_yg');
    ...
    // Support for the quiz_yg reference type.
