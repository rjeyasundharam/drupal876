...
namespace Drupal\quiz_yg\Plugin\Field\FieldWidget;
...
use Drupal\quiz_yg\QuizInterface;
use Drupal\quiz_yg\Plugin\EntityReferenceSelection\QuizSelection;
use Symfony\Component\Validator\ConstraintViolationInterface;
use Symfony\Component\Validator\ConstraintViolationListInterface;

/**
 * Plugin implementation of the 'entity_reference_revisions quiz_yg' widget.
 *
 * @FieldWidget(
 *   id = "quiz_yg",
 *   label = @Translation("Quiz EXPERIMENTAL"),
 *   description = @Translation("An experimental quiz_yg inline form widget."),
 *   field_types = {
...
class QuizWidget extends WidgetBase {
  ...
  /**
   * Action position is in the add quiz_yg place.
   */
  ...
  /**
   * Wrapper id to identify the quiz_yg.
   *
   * @var string
   */
  ...
  /**
   * Number of quiz_yg item on form.
   *
   * @var int
   */
  ...
  /**
   * Parents for the current quiz_yg.
   *
   * @var array
   */
  ...
  /**
   * Constructs a QuizWidget object.
   *
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['title_plural'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Plural Quiz Title'),
    ...
    $elements['edit_mode'] = array(
      ...
      '#description' => $this->t('The mode the quiz_yg is in by default.'),
    ...
    $elements['closed_mode'] = [
      ...
      '#description' => $this->t('How to display the quiz_yg, when the widget is closed. Preview will render the quiz_yg in the preview view mode and typically needs a custom admin theme.'),
    ...
    $elements['autocollapse'] = [
      ...
      '#description' => $this->t('When a quiz_yg is opened for editing, close others.'),
    ...
    $elements['closed_mode_threshold'] = [
      ...
      '#description' => $this->t('Number of items considered to leave quiz_yg open e.g the threshold is 3, if a quiz_yg has less than 3 items, leave it open.'),
      '#min' => 0,
      '#states' => [
        'invisible' => [
    ...
    $elements['add_mode'] = array(
      ...
      '#description' => $this->t('The way to add new Quiz.'),
    ...
    $elements['form_display_mode'] = array(
      ...
      '#description' => $this->t('The form display mode to use when rendering the quiz_yg form.'),
    ...
    $elements['default_quiz_yg_type'] = [
      '#type' => 'select',
      '#title' => $this->t('Default quiz_yg type'),
      '#empty_value' => '_none',
      '#default_value' => $this->getDefaultQuizTypeMachineName(),
      '#options' => $options,
      '#description' => $this->t('When creating a new host entity, a quiz_yg of this type is added.'),
    ];
   ...
   * \Drupal\quiz_yg\Plugin\Field\FieldWidget\QuizWidget::settingsSummary()
    ...
    switch($setting_name) {
      ...
      case 'features':
        $options = [
          ...
          // @see Drupal.behaviors.quiz_ygAddAboveButton
  ...
  public function settingsSummary() {
    ...
    if ($this->getDefaultQuizTypeLabelName() !== NULL) {
      $summary[] = $this->t('Default quiz_yg type: @default_quiz_yg_type', [
        '@default_quiz_yg_type' => $this->getDefaultQuizTypeLabelName()
      ]);
    }
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\quiz_yg\Entity\Quiz $quiz_yg_entity */
    $quiz_yg_entity = NULL;
    ...
    $item_mode = isset($widget_state['quiz_yg'][$delta]['mode']) ? $widget_state['quiz_yg'][$delta]['mode'] : 'edit';
    ...
    $show_must_be_saved_warning = !empty($widget_state['quiz_yg'][$delta]['show_warning']);

    if (isset($widget_state['quiz_yg'][$delta]['entity'])) {
      $quiz_yg_entity = $widget_state['quiz_yg'][$delta]['entity'];
    }
    elseif (isset($items[$delta]->entity)) {
      $quiz_yg_entity = $items[$delta]->entity;

      // We don't have a widget state yet, get from selector settings.
      if (!isset($widget_state['quiz_yg'][$delta]['mode'])) {
        ...
        elseif ($default_edit_mode == 'closed_expand_nested') {
          $item_mode = 'closed';
          $field_definitions = $quiz_yg_entity->getFieldDefinitions();

          // If the quiz_yg contains other quiz_yg, then open it.
          foreach ($field_definitions as $field_definition) {
            if ($field_definition->getType() == 'entity_reference_revisions' && $field_definition->getSetting('target_type') == 'quiz_yg') {
              $item_mode = 'edit';
              break;
            }
          }
    ...
    elseif (isset($widget_state['selected_bundle'])) {
      ...
      $quiz_yg_entity = $entity_type_manager->getStorage($target_type)->create(array(
        $bundle_key => $widget_state['selected_bundle'],
      ));
      $quiz_yg_entity->setParentEntity($host, $field_name);
    ...
    if ($quiz_yg_entity) {
      ...
      if (!$this->isTranslating) {
        ...
        $langcode_key = $quiz_yg_entity->getEntityType()->getKey('langcode');
        if ($quiz_yg_entity->get($langcode_key)->value != $langcode) {
          // If a translation in the given language already exists, switch to
          // that. If there is none yet, update the language.
          if ($quiz_yg_entity->hasTranslation($langcode)) {
            $quiz_yg_entity = $quiz_yg_entity->getTranslation($langcode);
          }
          else {
            $quiz_yg_entity->set($langcode_key, $langcode);
          }
        }
      }
      else {
        // If the node is being translated, the quiz_yg should be all open
        // when the form is not being rebuilt (E.g. when clicked on a quiz_yg
        ...
        if (!$quiz_yg_entity->hasTranslation($langcode)) {
          // Get the selected translation of the quiz_yg entity.
          $entity_langcode = $quiz_yg_entity->language()->getId();
          ...
          // valid scenario to have no quiz_yg items in the source version of
          // to an exception.
          if ($quiz_yg_entity->hasTranslation($source_langcode)) {
            $quiz_yg_entity = $quiz_yg_entity->getTranslation($source_langcode);
          }
          // The quiz_yg entity has no content translation source field if
          // no quiz_yg entity field is translatable, even if the host is.
          if ($quiz_yg_entity->hasField('content_translation_source')) {
            // Initialise the translation with source language values.
            $quiz_yg_entity->addTranslation($langcode, $quiz_yg_entity->toArray());
            $translation = $quiz_yg_entity->getTranslation($langcode);
            $manager = \Drupal::service('content_translation.manager');
            $manager->getTranslationMetadata($translation)->setSource($quiz_yg_entity->language()->getId());
          }
        }
        // If any quiz_yg type is translatable do not switch.
        if ($quiz_yg_entity->hasField('content_translation_source')) {
          // Switch the quiz_yg to the translation.
          $quiz_yg_entity = $quiz_yg_entity->getTranslation($langcode);
        }
      }

      // If untranslatable fields are hidden while translating, we are
      // translating the parent and the Quiz is open, then close the
      // Quiz if it does not have translatable fields.
      $translating_force_close = FALSE;
      if (\Drupal::moduleHandler()->moduleExists('content_translation')) {
        $manager = \Drupal::service('content_translation.manager');
        $settings = $manager->getBundleTranslationSettings('quiz_yg', $quiz_yg_entity->getQuizType()->id());
        if (!empty($settings['untranslatable_fields_hide']) && $this->isTranslating) {
          $translating_force_close = TRUE;
          $display = EntityFormDisplay::collectRenderDisplay($quiz_yg_entity, $this->getSetting('form_display_mode'));
          // Check if the quiz_yg has translatable fields.
          foreach (array_keys($display->get('content')) as $field) {
            if ($quiz_yg_entity->hasField($field)) {
              $field_definition = $quiz_yg_entity->get($field)->getFieldDefinition();
              // Check if we are referencing quiz_yg.
              $is_quiz_yg = ($field_definition->getType() == 'entity_reference_revisions' && $field_definition->getSetting('target_type') == 'quiz_yg');
              if ($is_quiz_yg || $field_definition->isTranslatable()) {
                $translating_force_close = FALSE;
                break;
              }
            }
      ...
      $element += array(
        ...
        '#quiz_yg_type' => $quiz_yg_entity->bundle(),
        'subform' => array(
          '#type' => 'container',
      ...
      $element['top'] = [
        ...
        // Section for quiz_yg type information.
        'type' => [
          '#type' => 'container',
          '#attributes' => ['class' => ['quiz_yg-type']],
        ],
        // Section for info icons.
        'icons' => [
          '#type' => 'container',
          '#attributes' => ['class' => ['quiz_yg-info']],
        ],
        'summary' => [
          '#type' => 'container',
          '#attributes' => ['class' => ['quiz_yg-summary']],
        ],
        // Quiz actions element for actions and dropdown actions.
        'actions' => [
      ...
      if (isset($item_bundles[$quiz_yg_entity->bundle()])) {
        $bundle_info = $item_bundles[$quiz_yg_entity->bundle()];

        $element['top']['type']['label'] = ['#markup' => $bundle_info['label']];

        // Type icon and label bundle.
        if ($icon_url = $quiz_yg_entity->type->entity->getIconUrl()) {
          $element['top']['type']['icon'] = [
            '#theme' => 'image',
            '#uri' => $icon_url,
            '#attributes' => [
              'class' => ['quiz_yg-type-icon'],
        ...
        $element['top']['type']['label'] = [
          '#markup' => '<span class="quiz_yg-type-label">' . $bundle_info['label'] . '</span>',
          '#weight' => 1,
        ];
        ...
        $widget_actions['dropdown_actions']['duplicate_button'] = [
          ...
          '#access' => $this->duplicateButtonAccess($quiz_yg_entity),
        ];
        ...
        if ($item_mode != 'remove') {
          $widget_actions['dropdown_actions']['remove_button'] = [
            ...
            '#weight' => 501,
            '#submit' => [[get_class($this), 'quiz_ygItemSubmit']],
            // Ignore all validation errors because deleting invalid quiz_yg
            // is allowed.
            ...
            '#access' => $this->removeButtonAccess($quiz_yg_entity),
            '#quiz_yg_mode' => 'remove',
          ];
        }

        if ($item_mode == 'edit') {
          if (isset($quiz_yg_entity)) {
            $widget_actions['actions']['collapse_button'] = [
              ...
              '#access' => $quiz_yg_entity->access('update') && !$translating_force_close,
              '#quiz_yg_mode' => 'closed',
              '#quiz_yg_show_warning' => TRUE,
              '#attributes' => [
                'class' => ['quiz_yg-icon-button', 'quiz_yg-icon-button-collapse'],
          ...
          $widget_actions['actions']['edit_button'] = $this->expandButton([
            ...
            '#quiz_yg_mode' => 'edit',
            '#attributes' => [
              'class' => ['quiz_yg-icon-button', 'quiz_yg-icon-button-edit'],
              'title' => $this->t('Edit'),
            ],
          ]);

          if ($show_must_be_saved_warning && $quiz_yg_entity->isChanged()) {
            $info['changed'] = [
              '#theme' => 'quiz_yg_info_icon',
              '#message' => $this->t('You have unsaved changes on this @title item.', ['@title' => $this->getSetting('title')]),
              '#icon' => 'changed',
            ];
          }

          if (!$quiz_yg_entity->access('view')) {
            $info['preview'] = [
              '#theme' => 'quiz_yg_info_icon',
        ...
        if (!$quiz_yg_entity->access('update')) {
        ...
        if (!$quiz_yg_entity->access('update') && $quiz_yg_entity->access('delete')) {
          $info['edit'] = [
            '#theme' => 'quiz_yg_info_icon',
        ...
        elseif (!$quiz_yg_entity->access('delete') && $quiz_yg_entity->access('update')) {
          $info['remove'] = [
            '#theme' => 'quiz_yg_info_icon',
      ...
      if (\Drupal::moduleHandler()->moduleExists('field_group')) {
        $context = [
          'entity_type' => $quiz_yg_entity->getEntityTypeId(),
          'bundle' => $quiz_yg_entity->bundle(),
          'entity' => $quiz_yg_entity,
          'context' => 'form',
      ...
      if ($item_mode == 'edit') {
        $display->buildForm($quiz_yg_entity, $element['subform'], $form_state);
        $hide_untranslatable_fields = $quiz_yg_entity->isDefaultTranslationAffectedOnly();
        ...
        foreach (Element::children($element['subform']) as $field) {
          if ($quiz_yg_entity->hasField($field)) {
            ...
            // those classes (quiz_yg-content and quiz_yg-behavior) to show
            // and hide elements, depending of the active perspective.
            // We need them to filter out entity reference revisions fields that
            // reference quiz_yg, cause otherwise we have problems with showing
            // and hiding the right fields in nested quiz_yg.
            $is_quiz_yg_field = FALSE;
            if ($field_definition->getType() == 'entity_reference_revisions') {
              // Check if we are referencing quiz_yg.
              if ($field_definition->getSetting('target_type') == 'quiz_yg') {
                $is_quiz_yg_field = TRUE;
              }
            }

            if (!$is_quiz_yg_field) {
              $element['subform'][$field]['#attributes']['class'][] = 'quiz_yg-content';
            }
            $translatable = $field_definition->isTranslatable();
            // Hide untranslatable fields when configured to do so except
            // quiz_yg fields.
            if (!$translatable && $this->isTranslating && !$is_quiz_yg_field) {
        ...
        $quiz_yg_type = $quiz_yg_entity->getQuizType();
        if ($quiz_yg_type && \Drupal::currentUser()->hasPermission('edit behavior plugin settings') && (!$this->isTranslating || !$hide_untranslatable_fields)) {
          $element['behavior_plugins']['#weight'] = -99;
          foreach ($quiz_yg_type->getEnabledBehaviorPlugins() as $plugin_id => $plugin) {
            $element['behavior_plugins'][$plugin_id] = [
              '#type' => 'container',
              '#group' => implode('][', array_merge($element_parents, ['quiz_yg_behavior'])),
            ];
            $subform_state = SubformState::createForSubform($element['behavior_plugins'][$plugin_id], $form, $form_state);
            if ($plugin_form = $plugin->buildBehaviorForm($quiz_yg_entity, $element['behavior_plugins'][$plugin_id], $subform_state)) {
              $element['behavior_plugins'][$plugin_id] = $plugin_form;
              // Add the quiz_yg-behavior class, so that we are able to show
              // and hide behavior fields, depending on the active perspective.
              $element['behavior_plugins'][$plugin_id]['#attributes']['class'][] = 'quiz_yg-behavior';
            }
          }
        }
      }
      elseif ($item_mode == 'closed') {
        ...
        if ($closed_mode_setting === 'preview') {
          // The closed quiz_yg is displayed as a rendered preview.
          $view_builder = $entity_type_manager->getViewBuilder('quiz_yg');

          $element['preview'] = $view_builder->view($quiz_yg_entity, 'preview', $quiz_yg_entity->language()->getId());
          $element['preview']['#access'] = $quiz_yg_entity->access('view');
        }
        else {
          // The closed quiz_yg is displayed as a summary.
          if ($quiz_yg_entity) {
            $summary = $quiz_yg_entity->getSummary();
            if (!empty($summary)) {
              $element['top']['summary']['fields_info'] = [
                '#markup' => $summary,
                '#prefix' => '<div class="quiz_yg-collapsed-description">',
                '#suffix' => '</div>',
                '#access' => $quiz_yg_entity->access('update') || $quiz_yg_entity->access('view'),
              ];
            }

            $info = array_merge($info, $quiz_yg_entity->getIcons());
          }
        }
      }
      else {
      ...
      $element['subform']['#attributes']['class'][] = 'quiz_yg-subform';
      $element['subform']['#access'] = $quiz_yg_entity->access('update');

      if ($item_mode == 'remove') {
        $element['#access'] = FALSE;
      }

      $widget_state['quiz_yg'][$delta]['entity'] = $quiz_yg_entity;
      $widget_state['quiz_yg'][$delta]['display'] = $display;
      $widget_state['quiz_yg'][$delta]['mode'] = $item_mode;
  ...
  /**
   * Builds an add quiz_yg button for opening of modal form.
   *
   * @param array $element
   *   Render element.
   */
  protected function buildModalAddForm(array &$element) {
    ...
    $element['#theme'] = 'quiz_yg_add_dialog';

    $element['add_modal_form_area'] = [
      '#type' => 'container',
      '#attributes' => [
        'class' => [
          'quiz_yg-type-add-modal',
          'first-button',
        ],
      ],
    ...
    // be used by sub-modules or other quiz_yg integration. CSS class is used
    // to support easier element selecting in JavaScript.
    $element['add_modal_form_area']['add_more_delta'] = [
      '#type' => 'hidden',
      '#attributes' => [
        'class' => [
          'quiz_yg-type-add-modal-delta',
        ],
      ],
    ];

    $element['#attached']['library'][] = 'quiz_yg/drupal.quiz_yg.modal';
    if ($this->isFeatureEnabled('add_above')) {
      $element['#attached']['library'][] = 'quiz_yg/drupal.quiz_yg.add_above_button';
    }
   ...
   *
   * @return array
   *   A list of arrays keyed by the quiz_yg type machine name with the following properties.
   *     - label: The label of the quiz_yg type.
   *     - weight: The weight of the quiz_yg type.
   */
  public function getAllowedTypes(FieldDefinitionInterface $field_definition = NULL) {
    ...
    if ($handler instanceof QuizSelection) {
  ...
  public function formMultipleElements(FieldItemListInterface $items, array &$form, FormStateInterface $form_state) {
    ...
    // Consider adding a default quiz_yg for new host entities.
    if ($max == 0 && $items->getEntity()->isNew()) {
      $default_type = $this->getDefaultQuizTypeMachineName();

      // Checking if default_type is not none and if is allowed.
      if ($default_type) {
        // Place the default quiz_yg.
        $target_type = $this->getFieldSetting('target_type');

        /** @var \Drupal\quiz_yg\QuizInterface $quiz_yg_entity */
        $quiz_yg_entity = $entity_type_manager->getStorage($target_type)->create([
          'type' => $default_type,
        ]);
        $quiz_yg_entity->setParentEntity($items->getEntity(), $field_name);
        $field_state['selected_bundle'] = $default_type;
        $display = EntityFormDisplay::collectRenderDisplay($quiz_yg_entity, $this->getSetting('form_display_mode'));
        $field_state['quiz_yg'][0] = [
          'entity' => $quiz_yg_entity,
          'display' => $display,
          'mode' => 'edit',
          'original_delta' => 1
    ...
    // If the parent entity is quiz_yg add the nested class if not then add
    // the perspective tabs.
    $field_prefix = strtr($this->fieldIdPrefix, '_', '-');
    if (count($this->fieldParents) == 0) {
      if ($items->getEntity()->getEntityTypeId() != 'quiz_yg') {
        $tabs = '<ul class="quiz_yg-tabs tabs primary clearfix"><li id="content" class="tabs__tab"><a href="#' . $field_prefix . '-values">Content</a></li><li id="behavior" class="tabs__tab"><a href="#' . $field_prefix . '-values">Behavior</a></li></ul>';
      }
    }
    if (count($this->fieldParents) > 0) {
      if ($items->getEntity()->getEntityTypeId() === 'quiz_yg') {
        $form['#attributes']['class'][] = 'quiz_yg-nested';
      }
    }
    $elements['#prefix'] = '<div class="is-horizontal quiz_yg-tabs-wrapper" id="' . $this->fieldWrapperId . '">' . $tabs;
    $elements['#suffix'] = '</div>';
    ...
    if (!empty($field_state['dragdrop'])) {
      ...
      $elements['#attached']['library'][] = 'quiz_yg/quiz_yg-dragdrop';
      //$elements['dragdrop_mode']['#button_type'] = 'primary';
      $elements['dragdrop'] = $this->buildNestedQuizFoDragDrop($form_state, NULL, []);
      return $elements;
    }
    ...
    if ($max > 0) {
      for ($delta = 0; $delta < $max; $delta++) {
        ...
        if ($element) {
          ...
          // Access for the top element is set to FALSE only when the quiz_yg
          // was removed. A quiz_yg that a user can not edit has access on
          // lower level.
    ...
    if ($header_actions) {
      ...
      // quiz_yg_preprocess_field_multiple_value_form().
    ...
    $elements['#attached']['library'][] = 'quiz_yg/drupal.quiz_yg.widget';
  ...
  /**
   * Returns a list of child quiz_yg for a given field to loop over.
   *
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The form state.
   * @param string $field_name
   *   The field name for which to find child quiz_yg.
   * @param \Drupal\quiz_yg\QuizInterface $quiz_yg
   *   The current quiz_yg.
   * @param array $array_parents
   *   The current field parent structure.
   *
   * @return \Drupal\quiz_yg\Entity\Quiz[]
   *   Child quiz_yg.
   */
  protected function getChildQuiz(FormStateInterface $form_state, $field_name, QuizInterface $quiz_yg = NULL, array $array_parents = []) {
    ...
    $entities = [];
    if ($child_field_state && isset($child_field_state['quiz_yg'])) {
      // Fetch the quiz_yg from the field state. Use the original delta
      // to get the right position. Also reorder the quiz_yg in the widget
      // state accordingly.
      $new_widget_quiz_yg = [];
      foreach ($child_field_state['quiz_yg'] as $child_delta => $child_field_item_state) {
        $entities[array_search($child_delta, $child_field_state['original_deltas'])] = $child_field_item_state['entity'];
        $new_widget_quiz_yg[array_search($child_delta, $child_field_state['original_deltas'])] = $child_field_item_state;
      }
      ksort($entities);

      // Set the orderd quiz_yg into the widget state and reset original
      // deltas.
      ksort($new_widget_quiz_yg);
      $child_field_state['quiz_yg'] = $new_widget_quiz_yg;
      $child_field_state['original_deltas'] = range(0, count($child_field_state['quiz_yg']) - 1);
      NestedArray::setValue($form_state->getStorage(), $current_parents, $child_field_state);
    }
    elseif ($quiz_yg) {
      // If there is no field state, return the quiz_yg directly from the
      // entity.
      foreach ($quiz_yg->get($field_name) as $child_delta => $item) {
        if ($item->entity) {
   ...
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The form state.
   * @param \Drupal\quiz_yg\QuizInterface|null $quiz_yg
   *   The parent quiz_yg, NULL for the initial call.
   * @param string[] $array_parents
   *   The array parents for nested quiz_yg.
   *
   * @return array
   *   The built form structure.
   */
  protected function buildNestedQuizFoDragDrop(FormStateInterface $form_state, QuizInterface $quiz_yg = NULL, array $array_parents = []) {
    // Look for nested elements.
    $elements = [];
    $field_definitions = [];
    if ($quiz_yg) {
      foreach ($quiz_yg->getFieldDefinitions() as $child_field_name => $field_definition) {
    ...
    foreach ($field_definitions as $child_field_name => $field_definition) {
      ...
      $elements[$child_field_name] = [
        '#type' => 'container',
        '#attributes' => ['class' => ['quiz_yg-dragdrop-wrapper']],
      ];

      // Only show a field label if there is more than one quiz_yg field.
      $label = count($field_definitions) > 1 || !$quiz_yg ? '<label><strong class="quiz_yg-dragdrop__label quiz_yg-dragdrop__label--field">' . $field_definition->getLabel() . '</strong></label>' : '';

      $elements[$child_field_name]['list'] = [
        '#type' => 'markup',
        '#prefix' => $label . '<ul class="quiz_yg-dragdrop__list" data-quiz_yg-dragdrop-cardinality="' . $cardinality . '" data-quiz_yg-dragdrop-allowed-types="' . $allowed_types . '" data-quiz_yg-dragdrop-path="' . $child_path . '">',
        '#suffix' => '</ul>',
      ];

      /** @var \Drupal\quiz_yg\Entity\Quiz $child_quiz_yg */
      foreach ($this->getChildQuiz($form_state, $child_field_name, $quiz_yg, $array_parents) as $child_delta => $child_quiz_yg) {
        $element = [];
        $element['top'] = [
          '#type' => 'container',
          '#attributes' => ['class' => ['quiz_yg-summary-wrapper']],
        ];
        $element['top']['quiz_yg_summary']['type'] = [
          '#markup' => '<strong class="quiz_yg-dragdrop__label quiz_yg-dragdrop__label--bundle">' . $child_quiz_yg->getQuizType()->label() . '</strong>',
        ];
        ...
        $element['_weight'] = array(
          ...
          '#attributes' => [
            'class' => ['quiz_yg-dragdrop__weight'],
          ]
        );
        ...
        $element['_path'] = [
          ...
          '#attributes' => [
            'class' => ['quiz_yg-dragdrop__path'],
          ]
        ];

        $summary_options = [];

        $element['#prefix'] = '<li class="quiz_yg-dragdrop__item" data-quiz_yg-dragdrop-bundle="' . $child_quiz_yg->bundle() . '"><a href="#" class="quiz_yg-dragdrop__handle"><span class="quiz_yg-dragdrop__icon"></span></a>';
        $element['#suffix'] = '</li>';
        $child_array_parents = array_merge($array_parents,  [$child_field_name, $child_delta]);

        if ($child_elements = $this->buildNestedQuizFoDragDrop($form_state, $child_quiz_yg, $child_array_parents)) {
        ...
        $element['top']['summary']['fields_info'] = [
          '#markup' => $child_quiz_yg->getSummary($summary_options),
          '#prefix' => '<div class="quiz_yg-collapsed-description">',
          '#suffix' => '</div>',
          '#access' => $child_quiz_yg->access('update') || $child_quiz_yg->access('view'),
        ];

        $info = $child_quiz_yg->getIcons();
        if (isset($info['count'])) {
  ...
  /**
   * Returns the available quiz_yg type.
   *
   * @return array
   *   Available quiz_yg types.
   */
  ...
  /**
   * Helper to create a quiz_yg UI message.
   *
  ...
  /**
   * Expand button base array into a quiz_yg widget action button.
   *
  ...
  public static function expandButton(array $button_base) {
    ...
    $button = $button_base + [
      '#type' => 'submit',
      '#theme_wrappers' => ['input__submit__quiz_yg_action'],
    ];
   ...
   */
  public static function getSubmitElementInfo(array $form, FormStateInterface $form_state, $position = QuizWidget::ACTION_POSITION_BASE) {
    $submit['button'] = $form_state->getTriggeringElement();

    // Go up in the form, to the widgets container.
    if ($position == QuizWidget::ACTION_POSITION_BASE) {
      $submit['element'] = NestedArray::getValue($form, array_slice($submit['button']['#array_parents'], 0, -2));
    }
    if ($position == QuizWidget::ACTION_POSITION_HEADER) {
      $submit['element'] = NestedArray::getValue($form, array_slice($submit['button']['#array_parents'], 0, -3));
    }
    elseif ($position == QuizWidget::ACTION_POSITION_ACTIONS) {
  ...
  protected function buildDropbutton(array $elements = []) {
    $build = [
      '#type' => 'container',
      '#attributes' => ['class' => ['quiz_yg-dropbutton-wrapper']],
    ];
  ...
  /**
   * Builds dropdown button for adding new quiz_yg.
   *
   * @return array
   *   The form element array.
   */
  protected function buildButtonsAddMode() {
    ...
    $quiz_yg_type_storage = \Drupal::entityTypeManager()->getStorage('quiz_yg_type');
    ...
    foreach ($options as $machine_name => $label) {
      ...
      if ($add_mode === 'modal' && $icon_url = $quiz_yg_type_storage->load($machine_name)->getIconUrl()) {
  ...
  /**
   * Builds list of actions based on quiz_yg type.
   *
   * @return array
   *   The form element array.
   */
  ...
  public static function addMoreAjax(array $form, FormStateInterface $form_state) {
    $submit = QuizWidget::getSubmitElementInfo($form, $form_state);
  ...
  public static function allActionsAjax(array $form, FormStateInterface $form_state) {
    $submit = QuizWidget::getSubmitElementInfo($form, $form_state, QuizWidget::ACTION_POSITION_HEADER);
    $element = $submit['element'];
  ...
  /**
   * Prepares the widget state to add a new quiz_yg at a specific position.
   *
   * In addition to the widget state change, also user input could be modified
   * to handle adding of a new quiz_yg at a specific position between existing
   * quiz_yg.
   *
   ...
   *   Path to quiz_yg field.
   * @param int|mixed $new_delta
   *   Delta position in list of quiz_yg, where new quiz_yg will be added.
   */
  protected static function prepareDeltaPosition(array &$widget_state, FormStateInterface $form_state, array $field_path, $new_delta) {
    // Increase number of items to create place for new quiz_yg.
    ...
    // Limit delta between 0 and "number of items" in quiz_yg widget.
  ...
  /**
   * Creates a duplicate of the quiz_yg entity.
   */
  public static function duplicateSubmit(array $form, FormStateInterface $form_state) {
    ...
    $entity = $widget_state['quiz_yg'][$original_button_delta]['entity'];
    ...
    // Create the duplicated quiz_yg and insert it below the original.
    $widget_state['quiz_yg'][] = [
      'entity' => $duplicate_entity,
      'display' => $widget_state['quiz_yg'][$original_button_delta]['display'],
      'mode' => 'edit',
    ];
  ...
  public static function quiz_ygItemSubmit(array $form, FormStateInterface $form_state) {
    $submit = QuizWidget::getSubmitElementInfo($form, $form_state, QuizWidget::ACTION_POSITION_ACTIONS);

    $new_mode = $submit['button']['#quiz_yg_mode'];

    if ($new_mode === 'edit') {
      $submit['widget_state'] = static::autocollapse($submit['widget_state']);
    }

    $submit['widget_state']['quiz_yg'][$submit['delta']]['mode'] = $new_mode;

    if (!empty($submit['button']['#quiz_yg_show_warning'])) {
      $submit['widget_state']['quiz_yg'][$submit['delta']]['show_warning'] = $submit['button']['#quiz_yg_show_warning'];
    }
   ...
   */
  protected static function reorderQuiz(FormStateInterface $form_state, $field_values_parents) {
    ...
    // Clear out all current quiz_yg keys in all nested quiz_yg widgets
    // as there might be fewer than before or none in a certain widget.
    $clear_quiz_yg = function ($field_storage) use (&$clear_quiz_yg) {
      foreach ($field_storage as $key => $value) {
        if ($key === '#fields') {
          foreach ($value as $field_name => $widget_state) {
            if (isset($widget_state['quiz_yg'])) {
              $field_storage['#fields'][$field_name]['quiz_yg'] = [];
            }
          }
        }
        else {
          $field_storage[$key] = $clear_quiz_yg($field_storage[$key]);
        }
      }
    ...
    // quiz_yg references in other fields.
    $new_field_storage['#fields'][$field_name]['quiz_yg'] = [];
    if (isset($new_field_storage[$field_name])) {
      $new_field_storage[$field_name] = $clear_quiz_yg($new_field_storage[$field_name]);
    }

    $reorder_quiz_yg = function ($reorder_values, $parents = [], FieldableEntityInterface $parent_entity = NULL) use ($complete_field_storage, &$new_field_storage, &$reorder_quiz_yg) {
        ...
        foreach ($values['list'] as $delta => $item_values) {
          ...
          if (!$key_exists && $parent_entity) {
            ...
            if (!NestedArray::getValue($new_field_storage, $widget_state_keys)) {
              NestedArray::setValue($new_field_storage, $widget_state_keys, ['quiz_yg' => []]);
            }
          }

          // Ensure the referenced quiz_yg will be saved.
          ...
          if (isset($item_values['dragdrop'])) {
            ...
            // it to an empty array in case all quiz_yg have been moved away
            // from it.
            foreach (array_keys($item_values['dragdrop']) as $sub_field_name) {
              ...
              if (!NestedArray::getValue($new_field_storage, $new_widget_state_keys)) {
                NestedArray::setValue($new_field_storage, $new_widget_state_keys, ['quiz_yg' => []]);
              }
            }

            $reorder_quiz_yg($item_values['dragdrop'], array_merge($parents, [$field_name, $delta, 'subform']), $item_state['entity']);
          }
        }
      }
    };
    $reorder_quiz_yg($field_values['dragdrop']);
      ...
      if (isset($field_storage['#fields'])) {
        foreach ($field_storage['#fields'] as $field_name => $widget_state) {
          if (isset($widget_state['quiz_yg'])) {
            ...
            // If the parent field does not exist but we have quiz_yg in
            // widget state, something went wrong and we have a mismatch.
            // Throw an exception.
            if (!$parent_entity->hasField($field_name) && !empty($widget_state['quiz_yg'])) {
              throw new \LogicException('Reordering quiz_yg resulted in quiz_yg on non-existing field ' . $field_name . ' on parent entity ' . $parent_entity->getEntityTypeId() . '/' . $parent_entity->id());
            }

            // Sort the quiz_yg by key so that they will be assigned to
            // the entity in the right order. Reset the deltas.
            ksort($widget_state['quiz_yg']);
            $widget_state['quiz_yg'] = array_values($widget_state['quiz_yg']);

            $original_deltas = range(0, count($widget_state['quiz_yg']) - 1);
            $field_storage['#fields'][$field_name]['original_deltas'] = $original_deltas;
            $field_storage['#fields'][$field_name]['items_count'] = count($widget_state['quiz_yg']);
            $field_storage['#fields'][$field_name]['real_item_count'] = count($widget_state['quiz_yg']);

            // Update the parent entity and point to the new children, if the
            // parent field does not exist, we also have no quiz_yg, so
            // we can just skip this, this is a dead leaf after re-ordering.
            // @todo Clean this up somehow?
            if ($parent_entity->hasField($field_name)) {
              $parent_entity->set($field_name, array_column($widget_state['quiz_yg'], 'entity'));

              // Next process that field recursively.
              foreach (array_keys($widget_state['quiz_yg']) as $delta) {
  ...
  public function elementValidate($element, FormStateInterface $form_state, $form) {
    ...
    if (isset($widget_state['quiz_yg'][$delta]['entity'])) {
      /** @var \Drupal\quiz_yg\QuizInterface $quiz_yg_entity */
      $entity = $widget_state['quiz_yg'][$delta]['entity'];

      /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $display */
      $display = $widget_state['quiz_yg'][$delta]['display'];

      if ($widget_state['quiz_yg'][$delta]['mode'] == 'edit') {
        // Extract the form values on submit for getting the current quiz_yg.
        $display->extractFormValues($entity, $element['subform'], $form_state);

        // Validate all enabled behavior plugins.
        $quiz_yg_type = $entity->getQuizType();
        if (\Drupal::currentUser()->hasPermission('edit behavior plugin settings')) {
          foreach ($quiz_yg_type->getEnabledBehaviorPlugins() as $plugin_id => $plugin_values) {
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    if (!empty($widget_state['dragdrop'])) {
      ...
      static::reorderQuiz($form_state, $path);
      ...
      $values = [];
      foreach ($widget_state['quiz_yg'] as $delta => $quiz_yg_state) {
        $values[$delta]['entity'] = $quiz_yg_state['entity'];
      }
      return $values;
    }

    foreach ($values as $delta => &$item) {
      if (isset($widget_state['quiz_yg'][$item['_original_delta']]['entity'])
        && $widget_state['quiz_yg'][$item['_original_delta']]['mode'] != 'remove') {
        /** @var \Drupal\quiz_yg\QuizInterface $quiz_yg_entity */
        $quiz_yg_entity = $widget_state['quiz_yg'][$item['_original_delta']]['entity'];

        /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $display */
        $display = $widget_state['quiz_yg'][$item['_original_delta']]['display'];
        if ($widget_state['quiz_yg'][$item['_original_delta']]['mode'] == 'edit') {
          $display->extractFormValues($quiz_yg_entity, $element[$item['_original_delta']]['subform'], $form_state);
        }
        // A content entity form saves without any rebuild. It needs to set the
        // language to update it in case of language change.
        $langcode_key = $quiz_yg_entity->getEntityType()->getKey('langcode');
        if ($quiz_yg_entity->get($langcode_key)->value != $form_state->get('langcode')) {
          // If a translation in the given language already exists, switch to
          // that. If there is none yet, update the language.
          if ($quiz_yg_entity->hasTranslation($form_state->get('langcode'))) {
            $quiz_yg_entity = $quiz_yg_entity->getTranslation($form_state->get('langcode'));
          }
          else {
            $quiz_yg_entity->set($langcode_key, $form_state->get('langcode'));
          }
        }
        if (isset($item['behavior_plugins'])) {
          ...
          foreach ($quiz_yg_type->getEnabledBehaviorPlugins() as $plugin_id => $plugin_values) {
            ...
            if (isset($element[$original_delta]) && isset($element[$original_delta]['behavior_plugins'][$plugin_id]) && $form_state->getCompleteForm() && \Drupal::currentUser()->hasPermission('edit behavior plugin settings')) {
              ...
              if (isset($item['behavior_plugins'][$plugin_id])) {
                $plugin_values->submitBehaviorForm($quiz_yg_entity, $item['behavior_plugins'][$plugin_id], $subform_state);
              }
            }
          }
        }
        ...
        if ($widget_state['quiz_yg'][$item['_original_delta']]['mode'] === 'edit') {
          $display->validateFormValues($quiz_yg_entity, $element[$item['_original_delta']]['subform'], $form_state);
        }
        // Assume that the entity is being saved/previewed, in this case,
        // validate even the closed quiz_yg. If there are validation errors,
        ...
        elseif ($form_state->getLimitValidationErrors() === NULL) {
          $violations = $quiz_yg_entity->validate();
        ...
        $quiz_yg_entity->setNeedsSave(TRUE);
        $item['entity'] = $quiz_yg_entity;
        $item['target_id'] = $quiz_yg_entity->id();
        $item['target_revision_id'] = $quiz_yg_entity->getRevisionId();
      }
      // If our mode is remove don't save or reference this entity.
      // @todo: Maybe we should actually delete it here?
      elseif (isset($widget_state['quiz_yg'][$item['_original_delta']]['mode']) && $widget_state['quiz_yg'][$item['_original_delta']]['mode'] == 'remove') {
        $item['target_id'] = NULL;
  ...
  public static function addTranslatabilityClue(array $element, FormStateInterface $form_state) {
    ...
    // nested quiz_yg or similar nested field types.
  ...
  /**
   * Returns the default quiz_yg type.
   *
   * @return string
   *   Label name for default quiz_yg type.
   */
  protected function getDefaultQuizTypeLabelName() {
    if ($this->getDefaultQuizTypeMachineName() !== NULL) {
      $allowed_types = $this->getAllowedTypes();
      return $allowed_types[$this->getDefaultQuizTypeMachineName()]['label'];
    }
  ...
  /**
   * Returns the machine name for default quiz_yg type.
   *
   * @return string
   *   Machine name for default quiz_yg type.
   */
  protected function getDefaultQuizTypeMachineName() {
    $default_type = $this->getSetting('default_quiz_yg_type');
    $allowed_types = $this->getAllowedTypes();
    if ($default_type && isset($allowed_types[$default_type])) {
      return $default_type;
    }
    // Check if the user explicitly selected not to have any default Quiz
  ...
  /**
   * Counts the number of quiz_yg in a certain mode in a form substructure.
   *
   * @param array $widget_state
   ...
   *
   * @return int
   *   The number of quiz_yg is the given mode.
   */
  protected function getNumberOfQuizInMode(array $widget_state, $mode) {
    if (!isset($widget_state['quiz_yg'])) {
      return 0;
    }

    $quiz_yg_count = 0;
    foreach ($widget_state['quiz_yg'] as $quiz_yg) {
      if ($quiz_yg['mode'] == $mode) {
        $quiz_yg_count++;
      }
    }

    return $quiz_yg_count;
  }
  ...
  public static function isApplicable(FieldDefinitionInterface $field_definition) {
    ...
    $quiz_yg_type = \Drupal::entityTypeManager()->getDefinition($target_type);
    if ($quiz_yg_type) {
      return $quiz_yg_type->entityClassImplements(QuizInterface::class);
    }
  ...
  public function buildHeaderActions(array $field_state, FormStateInterface $form_state) {
    ...
    if (empty($this->fieldParents)) {
      ...
      $library = $library_discovery->getLibraryByName('quiz_yg', 'quiz_yg-dragdrop');
      if ($this->realItemCount > 0 && ($library || \Drupal::state()->get('quiz_yg_test_dragdrop_force_show', FALSE))) {
    ...
    if ($this->fieldDefinition->getType() == 'entity_reference_revisions' &&  $this->realItemCount > 1 && $this->isFeatureEnabled('collapse_edit_all')) {
      ...
      $mode = isset($field_state['quiz_yg'][0]['mode']) ? $field_state['quiz_yg'][0]['mode'] : $this->settings['edit_mode'];
    ...
    // Add quiz_yg_header flag which we use later in preprocessor to move
    // header actions to table header.
    if ($actions) {
      // Set actions.
      $actions['#type'] = 'quiz_yg_actions';
      $actions['#quiz_yg_header'] = TRUE;
    }
  ...
  /**
   * Loops through all quiz_yg and change mode for each quiz_yg instance.
   *
   * @param array $form
   *   Current form state.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public static function changeAllEditModeSubmit(array $form, FormStateInterface $form_state) {
    ...
    // Change edit mode for each quiz_yg.
    foreach ($submit['widget_state']['quiz_yg'] as $delta => &$quiz_yg) {
      if ($submit['widget_state']['quiz_yg'][$delta]['mode'] !== 'remove') {
        $submit['widget_state']['quiz_yg'][$delta]['mode'] = $submit['button']['#quiz_yg_mode'];
        if (!empty($submit['button']['#quiz_yg_show_warning'])) {
          $submit['widget_state']['quiz_yg'][$delta]['show_warning'] = $submit['button']['#quiz_yg_show_warning'];
        }
      }
    }

    if ($submit['widget_state']['autocollapse_default'] == 'all') {
      if ($submit['button']['#quiz_yg_mode'] === 'edit') {
        $submit['widget_state']['autocollapse'] = 'none';
      }
      elseif ($submit['button']['#quiz_yg_mode'] === 'closed') {
  ...
  /**
   * Returns a state with all quiz_yg closed, if autocollapse is enabled.
   *
   * @param array $widget_state
   *   The current widget state.
   *
   * @return array
   *   The widget state altered by closing all quiz_yg.
   */
  public static function autocollapse(array $widget_state) {
    if ($widget_state['real_item_count'] > 0 && $widget_state['autocollapse'] !== 'none') {
      foreach ($widget_state['quiz_yg'] as $delta => $value) {
        if ($widget_state['quiz_yg'][$delta]['mode'] === 'edit') {
          $widget_state['quiz_yg'][$delta]['mode'] = 'closed';
        }
      }
    }
   ...
   *
   * @param \Drupal\quiz_yg\QuizInterface $quiz_yg
   *   Quiz entity to check.
   *
   * @return bool
   *   TRUE if we can remove quiz_yg, otherwise FALSE.
   */
  protected function removeButtonAccess(QuizInterface $quiz_yg) {
    if (!$quiz_yg->access('delete')) {
      return FALSE;
    }
   ...
   *
   * @return bool
   *   TRUE if we can duplicate the quiz_yg, otherwise FALSE.
   */
  protected function duplicateButtonAccess(QuizInterface $quiz_yg) {
    if (!$this->isFeatureEnabled('duplicate')) {
      return FALSE;
    }

    if (!$quiz_yg->access('update')) {
      return FALSE;
    }
