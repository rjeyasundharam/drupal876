...
/**
 * Plugin implementation of the 'entity_reference quiz_yg' widget.
 *
 * We hide add / remove buttons when translating to avoid accidental loss of
 * data because these actions effect all languages.
 *
 * @FieldWidget(
 *   id = "entity_reference_quiz_yg",
 *   label = @Translation("Quiz Classic"),
 *   description = @Translation("A quiz_yg inline form widget."),
 *   field_types = {
 *     "entity_reference_revisions"
 *   }
 * )
 */
class InlineQuizWidget extends WidgetBase {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['edit_mode'] = array(
      ...
      '#description' => $this->t('The mode the quiz_yg is in by default. Preview will render the quiz_yg in the preview view mode.'),
      '#options' => array(
        'open' => $this->t('Open'),
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    elseif (isset($widget_state['selected_bundle'])) {
      ...
      $quiz_yg_entity = $entity_manager->getStorage($target_type)->create(array(
    ...
    if ($item_mode == 'closed') {
      // Validate closed quiz_yg and expand if needed.
    ...
    if ($quiz_yg_entity) {
      ...
      if (isset($item_bundles[$quiz_yg_entity->bundle()])) {
        ...
        $element['top']['quiz_yg_type_title'] = array(
          '#type' => 'container',
          '#weight' => 0,
          '#attributes' => array(
            'class' => array(
        ...
        $element['top']['quiz_yg_type_title']['info'] = array(
        ...
        $button_access = $quiz_yg_entity->access('delete') && !$this->isTranslating;
        if ($item_mode != 'remove') {
          $links['remove_button'] = [
            '#type' => 'submit',
            '#value' => $this->t('Remove'),
        ...
        if ($item_mode == 'edit') {
          ...
          if (isset($items[$delta]->entity) && ($default_edit_mode == 'preview' || $default_edit_mode == 'closed')) {
            $links['collapse_button'] = array(
              ...
              '#weight' => 499,
              '#submit' => array(array(get_class($this), 'quiz_ygItemSubmit')),
              '#delta' => $delta,
              '#limit_validation_errors' => [array_merge($parents, [$field_name, 'add_more'])],
              '#ajax' => array(
              ...
              '#access' => $quiz_yg_entity->access('update'),
              '#prefix' => '<li class="collapse">',
              '#suffix' => '</li>',
              '#quiz_yg_mode' => 'collapsed',
          ...
          $info['edit_button_info'] = array(
            ...
            '#attributes' => ['class' => ['messages', 'messages--warning']],
            '#access' => !$quiz_yg_entity->access('update') && $quiz_yg_entity->access('delete'),
          );
          ...
          $info['remove_button_info'] = array(
            ...
            '#attributes' => ['class' => ['messages', 'messages--warning']],
            '#access' => !$quiz_yg_entity->access('delete') && $quiz_yg_entity->access('update'),
          );
        ...
        elseif ($item_mode == 'preview' || $item_mode == 'closed') {
          ...
          $info['preview_info'] = array(
            ...
            '#attributes' => ['class' => ['messages', 'messages--warning']],
            '#access' => !$quiz_yg_entity->access('view'),
          );
        ...
        elseif ($item_mode == 'remove') {
          ...
          $element['top']['quiz_yg_type_title']['info'] = [
        ...
        if (count($links)) {
          ...
          if ($show_links > 0) {
            ...
            if ($show_links > 1) {
              $element['top']['links']['#theme_wrappers'] = array('dropbutton_wrapper', 'quiz_yg_dropbutton_wrapper');
              ...
              $element['top']['links']['#theme_wrappers'] = array('quiz_yg_dropbutton_wrapper');
      ...
      if ($item_mode == 'edit') {
        ...
        foreach (Element::children($element['subform']) as $field) {
          if ($quiz_yg_entity->hasField($field)) {
            $field_definition = $quiz_yg_entity->getFieldDefinition($field);
            $translatable = $quiz_yg_entity->{$field}->getFieldDefinition()->isTranslatable();
      ...
      elseif ($item_mode == 'preview') {
        ...
        $element['preview'] = entity_view($quiz_yg_entity, 'preview', $quiz_yg_entity->language()->getId());
      ...
      elseif ($item_mode == 'closed') {
        ...
        if ($quiz_yg_entity) {
          ...
          $element['top']['quiz_yg_summary']['fields_info'] = [
            '#markup' => $summary,
  ...
  public function formMultipleElements(FieldItemListInterface $items, array &$form, FormStateInterface $form_state) {
    ...
    $elements['#attached']['library'][] = 'quiz_yg/drupal.quiz_yg.admin';
  ...
  protected function buildButtonsAddMode() {
    ...
    $add_more_elements = [
      '#type' => 'container',
      '#theme_wrappers' => ['quiz_yg_dropbutton_wrapper'],
    ];
    $field_name = $this->fieldDefinition->getName();
    $title = $this->fieldDefinition->getLabel();
   ...
   *
   * Since the quiz_yg field is not set as translatable, the item language
  ...
  public static function quiz_ygItemSubmit(array $form, FormStateInterface $form_state) {
    ...
    $widget_state['quiz_yg'][$delta]['mode'] = $button['#quiz_yg_mode'];

    if (!empty($button['#quiz_yg_show_warning'])) {
      $widget_state['quiz_yg'][$delta]['show_warning'] = $button['#quiz_yg_show_warning'];
    }
  ...
  public function multipleElementValidate(array $elements, FormStateInterface $form_state, array $form) {
    ...
    $remove_mode_item_count = $this->getNumberOfQuizInMode($widget_state, 'remove');
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    foreach ($values as $delta => &$item) {
      ...
      elseif($widget_state['quiz_yg'][$item['_original_delta']]['mode'] == 'remove' || $widget_state['quiz_yg'][$item['_original_delta']]['mode'] == 'removed') {
        $item['target_id'] = NULL;
   ...
   *
   * @return string $default_quiz_yg_type
  ...
  public static function isApplicable(FieldDefinitionInterface $field_definition) {
    ...
    if ($quiz_yg_type) {
      return $quiz_yg_type->isSubclassOf(QuizInterface::class);
    }
