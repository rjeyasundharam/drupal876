...
use Drupal\quiz_yg\Controller\QuizFormController;
...
class QuizForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state,$node='') {
    ...
    foreach ($quiz['field_questions'] as $key => $value) {
      ...
      if($ptype=='multichoice')
        $form['question'][$key]=$qcon->getMultichoiceForm($quiz);
      elseif($ptype=='matching')
        $form['question'][$key]=$qcon->getMatchingForm($quiz);
      elseif($ptype=='t_or_f')
        $form['question'][$key]=$qcon->getTorFForm($quiz);
    }
    $form['quiz_id'] = [
      '#type' => 'hidden',
      '#value' => $node,
    ...
    $form['#attached']['library'][] = 'quiz_yg/quiz_yg_timer';
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $quiz_id=$values['quiz_id'];
    $url = Url::fromRoute('quiz_yg.quiz_result_id', [
      'quiz_id' => $quiz_id,
      'attempt_id'=>$attempt_id
    ]);
