 ...
 *   handlers = {
 *     "view_builder" = "Drupal\quiz_yg\QuizViewBuilder",
 *     "access" = "Drupal\quiz_yg\QuizAccessControlHandler",
 *     "storage_schema" = "Drupal\quiz_yg\QuizStorageSchema",
 *     "form" = {
 ...
 *   },
 *   base_table = "quiz_yg_item",
 *   data_table = "quiz_yg_item_field_data",
 *   revision_table = "quiz_yg_item_revision",
 *   revision_data_table = "quiz_yg_item_revision_field_data",
 *   translatable = TRUE,
 ...
 *   },
 *   bundle_entity_type = "quiz_yg_type",
 *   field_ui_base_route = "entity.quiz_yg_type.edit_form",
 ...
 *     "field_config" = {
 *       "settings" = {
 *         "handler" = "default:quiz_yg"
 *       }
 *     },
 *     "entity_form_display" = {
 *       "type" = "entity_reference_quiz_yg"
 *     },
 *     "entity_view_display" = {
...
class Quiz extends ContentEntityBase implements QuizInterface {
  ...
  /**
   * The behavior plugin data for the quiz_yg entity.
   *
   * @var array
   */
    ...
    if ($parent = $this->getParentEntity()) {
      ...
      foreach ($values as $key => $value) {
        if ($value->entity->id() == $this->id()) {
          ...
          // A previous or draft revision or a deleted stale Quiz.
  ...
  public static function baseFieldDefinitions(EntityTypeInterface $entity_type) {
    ...
    $fields['langcode'] = BaseFieldDefinition::create('language')
      ...
      ->setDescription(t('The quiz_yg entity language code.'))
      ->setRevisionable(TRUE);

    $fields['uid'] = BaseFieldDefinition::create('entity_reference')
      ->setLabel(t('Authored by'))
      ->setDescription(t('The user ID of the quiz_yg author.'))
      ->setRevisionable(TRUE)
      ->setSetting('target_type', 'user')
      ->setSetting('handler', 'default')
      ->setDefaultValueCallback('Drupal\quiz_yg\Entity\Quiz::getCurrentUserId')
 ...
 public function createDuplicate() {
   ...
   // Loop over entity fields and duplicate nested quiz_yg.
   foreach ($duplicate->getFields() as $field) {
     if ($field->getFieldDefinition()->getType() == 'entity_reference_revisions') {
       if ($field->getFieldDefinition()->getTargetEntityTypeId() == "quiz_yg") {
  ...
  public function getSummary(array $options = []) {
    ...
    $this->summaryCount = 0;
    $summary = [];
    $components = entity_get_form_display('quiz_yg', $this->getType(), 'default')->getComponents();
    ...
    foreach (array_keys($components) as $field_name) {
      ...
      // keeps performance while building the quiz_yg summary.
      ...
      if ($field_definition->getType() == 'entity_reference_revisions') {
        // Decrease the depth, since we are entering a nested quiz_yg.
  ...
  public function getIcons(array $options = []) {
    ...
    if ($this->summaryCount) {
      $icons['count'] = [
        ...
        '#prefix' => '<span class="quiz_yg-badge" title="' . (string) \Drupal::translation()->formatPlural($this->summaryCount, '1 child', '@count children') . '">',
        '#suffix' => '</span>',
      ];
    }
  ...
  public function isChanged() {
    ...
    /** @var \Drupal\quiz_yg\QuizInterface $original */
  ...
  /**
   * Returns summary for file quiz_yg.
   *
  ...
  /**
   * Returns summary for nested quiz_yg.
   *
   * @param string $field_name
   *   Field definition id for quiz_yg.
   * @param array $options
   *   (optional) An associative array of additional options.
   *   See \Drupal\quiz_yg\QuizInterface::getSummary() for all of the
   *   available options.
   *
   * @return string
   *   Short summary for nested Quiz type
  ...
  protected function getNestedSummary($field_name, array $options) {
    ...
    $quiz_yg_summary = implode(', ', $summary);
    return $quiz_yg_summary;
  }

  /**
   * Returns summary for all text type quiz_yg.
   *
   * @param string $field_name
   *   Field definition id for quiz_yg.
   * @param \Drupal\Core\Field\FieldDefinitionInterface $field_definition
   *   Field definition for quiz_yg.
   *
   * @return string
   *   Short summary for text quiz_yg.
   */
