...
/**
 * Access controller for the quiz_yg entity.
 *
 * @see \Drupal\quiz_yg\Entity\Quiz.
 */
class QuizAccessControlHandler extends EntityAccessControlHandler implements EntityHandlerInterface {
  ...
  protected function checkAccess(EntityInterface $paragraph, $operation, AccountInterface $account) {
    ...
    /** @var \Drupal\quiz_yg\Entity\Quiz $paragraph */
    $config = $this->configFactory->get('quiz_yg.settings');
    if ($operation === 'view') {
      $access_result = AccessResult::allowedIf($paragraph->isPublished() || ($account->hasPermission('view unpublished quiz_yg') && $config->get('show_unpublished')))->addCacheableDependency($config);
    }
    else {
    ...
    if ($paragraph->getParentEntity() != NULL) {
      ...
      if ($paragraph->getParentEntity()->getEntityTypeId() != 'quiz_yg_library_item') {
