...
class QuizResultController extends ControllerBase {
  ...
  public function getUserAllQuizResult($node) {
    ...
    if(in_array("administrator", $user_roles)){
      $query = \Drupal::entityQuery('node')
        ->condition('status', 1)
        ->condition('type', 'quiz_attempt')
        ->condition('field_quiz_id', $quiz_id);
      ...
      foreach ($attempts as $attempt_id) {
        $query = \Drupal::entityQuery('node')
        ->condition('status', 1)
        ->condition('type', 'quiz_result')
        ->condition('field_quiz_id', $quiz_id)
        ->condition('field_attempt_id', $attempt_id);
        $qresults = $query->execute();
        $tot=$out=0;
        foreach ($qresults as $qresult_id) {
          $qu=Node::load($qresult_id);
          $tot=$tot+$qu->field_score->value;
          $out++;
        }
        $attempt=Node::load($attempt_id);
        $score=$tot." / ".$out;
        $init = $attempt->field_time_taken->value;
        ...
        $rows[]=[
          'serial'=>$i++,
          'attempt_date'=>format_date($attempt->created->value),
          'score'=>$score,
  ...
  public function getUserQuizResult($quiz_id,$attempt_id) {
    ...
    ->condition('type', 'quiz_result')
    ->condition('field_quiz_id', $quiz_id)
    ->condition('field_user_id', $current_user->id())
    ->condition('field_attempt_id', $attempt_id);
    $nids = $query->execute();
    foreach ($nids as $nid) {
      $qu=Node::load($nid);
      $result[$qu->field_question_no->value][]=[
      'result_id'=>$nid,
      'attempt_id'=>$qu->field_attempt_id->target_id,
      'quiz_id'=>$que->field_quiz_id->target_id,
      'question_no'=>$qu->field_question_no->value,
      'question_id'=>$qu->field_question_id->value,
      'user_id'=>$qu->field_user_id->target_id,
      'answer'=>$qu->field_user_answer->value,
      'correct_answer'=>$qu->field_correct_answer->value,
      'correct'=>$qu->field_correct_answer->value,
      'created'=>$qu->created->value,
      'score'=>$qu->field_score->value,
     ];
     $created=$qu->created->value;
     $tot=$tot+$qu->field_score->value;
     $out++;
